...
def list_protein_references_by_gene(gene_id: str) -> List[Dict[str, Any]]:
    sb = get_supabase_client()
    res = (
        sb.table("protein_reference")
        .select(PROTEIN_REFERENCE_SELECT)
        .eq("gene_id", gene_id)
...
def get_protein_reference_by_id(protein_reference_id: str) -> Optional[Dict[str, Any]]:
    sb = get_supabase_client()
    res = (
        sb.table("protein_reference")
        .select(PROTEIN_REFERENCE_SELECT)
...
def upsert_protein_reference(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Some client versions do not expose upsert() cleanly.
        existing = None
        gid = payload.get("gene_id")
        tid = payload.get("transcript_id")
        if gid and tid:
            rows = list_protein_references_by_gene(str(gid))
    ...
    if not row:
        rows = list_protein_references_by_gene(str(payload.get("gene_id") or ""))
        row = next((r for r in rows if str(r.get("transcript_id")) == str(payload.get("transcript_id") or "")), None)
    if not row:
...
def clear_default_structure_flags(protein_reference_id: str) -> None:
    sb = get_supabase_client()
    sb.table("protein_structure_asset").update({"is_default": False}).eq("protein_reference_id", protein_reference_id).execute()
