...
def _compare_sequences_ignore_n(db_seq: str, ref_seq: str, *, max_show: int = 20) -> Dict[str, Any]:
    ...
    We additionally report the *covered span* (first..last A/C/G/T in DB) so you can
    distinguish:
      - long prefix/suffix padding (covered_rate < 1 but covered_rate_span ~ 1), vs
      - internal missing gaps (covered_rate_span < 1).
    """
    ...
    if covered_positions:
        ...
        span_len = int(span_end - span_start + 1)

        prefix_masked = int(span_start)
        suffix_masked = int(L - span_end - 1)

        # internal masked bases inside the covered span
        internal_masked = 0
        gap_intervals: List[Dict[str, Any]] = []
        in_gap = False
        gap_s = None
        for i in range(span_start, span_end + 1):
            if str(db_seq[i]).upper() in "ACGT":
                if in_gap:
                    gap_e = i - 1
                    gap_intervals.append(
                        {
                            "start_gene0": int(gap_s),
                            "end_gene0": int(gap_e),
                            "length": int(gap_e - gap_s + 1),
                        }
                    )
                    in_gap = False
                    gap_s = None
                ...
                if not in_gap:
                    in_gap = True
                    gap_s = i
        if in_gap and gap_s is not None:
            gap_e = span_end
            gap_intervals.append({"start_gene0": int(gap_s), "end_gene0": int(gap_e), "length": int(gap_e - gap_s + 1)})

        covered_rate_span = float(covered / max(1, span_len))
    else:
        span_start = None
        span_end = None
        span_len = 0
        prefix_masked = int(L)
        suffix_masked = 0
        internal_masked = int(L)
        gap_intervals = []
        covered_rate_span = 0.0

    mismatch_rate_on_covered = float(mismatch_count / max(1, covered))

    return {
        "ok": mismatch_count == 0,
        "len": int(L),
        "covered_bases": int(covered),
        "covered_rate": float(covered / max(1, L)),
        "covered_span_gene0_start": span_start,
        "covered_span_gene0_end": span_end,
        "covered_span_len": int(span_len),
        "covered_rate_span": covered_rate_span,
        "masked_prefix_len": int(prefix_masked),
        "masked_suffix_len": int(suffix_masked),
        "masked_internal_bases": int(internal_masked),
        "masked_internal_intervals": gap_intervals[:20],
        "mismatch_count": int(mismatch_count),
        "mismatch_rate_on_covered": mismatch_rate_on_covered,
        "mismatch_samples": mismatches,
    }

def _parse_canonical_exon_numbers(row: Any) -> Optional[List[int]]:
    """Parse canonical exon numbers from annotation row (if present).

    refannotation_with_canonical.tsv may contain a column like 'canonical_exon_numbers'
    (comma-separated, possibly with trailing comma). We treat numbers as 1-based indices
    into the EXON_START/EXON_END arrays.
    """
    keys = [
    ...
    for k in keys:
        try:
            ...
            raw = row[k] if k in row else None
        if raw is None:
            continue
        # handle NaN
        ...
        if s == "" or s.lower() in {"nan", "none"}:
            raw = None
            continue
        # allow formats like "2,4,6," or "[2,4,6]"
...
def _expected_exons_gene0_from_annotation(row: Any) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    """Return (exon_start_gene0, exon_end_gene0) in transcript order.

    IMPORTANT:
      - If 'canonical_exon_numbers' exists, we subset EXON_START/EXON_END to those exons
        and re-number them as exon1..exonN in transcript order.
      - This matches how you populate DB regions for the canonical transcript.
    """
    ...
    original_exon_count = int(starts.size)

    if starts.size == 0:
        meta = {
            "used_canonical_exon_numbers": None,
            "original_exon_count_annotation": original_exon_count,
            "effective_exon_count_annotation": 0,
        }
        return np.asarray([], dtype=int), np.asarray([], dtype=int), meta

    canonical_nums = _parse_canonical_exon_numbers(row)
    used_nums: Optional[List[int]] = None
    if canonical_nums:
        idx = [n - 1 for n in canonical_nums if 1 <= n <= original_exon_count]
        if idx:
            starts = starts[np.asarray(idx, dtype=int)]
            ends = ends[np.asarray(idx, dtype=int)]
            used_nums = list(canonical_nums)
    ...
    meta = {
        "used_canonical_exon_numbers": used_nums,
        "original_exon_count_annotation": original_exon_count,
        "effective_exon_count_annotation": int(s_gene0.size),
        "note": "Annotation exons are ordered in transcript order (+ asc, - desc).",
    }
    return s_gene0.astype(int), e_gene0.astype(int), meta
...
def _exon_boundary_check(db_exons: List[Dict[str, Any]], row: Any) -> Dict[str, Any]:
    ...
    We compare against the *canonical* exon set if the annotation row provides
    canonical_exon_numbers. This prevents false mismatches for genes like LMNA where
    EXON_START/END may include more than the canonical transcript exons.
    """
    exp_s, exp_e, meta = _expected_exons_gene0_from_annotation(row)
    if exp_s.size == 0:
        return {"ok": False, "reason": "annotation_has_no_exons", **meta}
...
def main() -> None:
    ...
    for did in disease_ids:
        ...
        ref_seq = genome.fetch_seq(chrom, tx_start_1b - 1, tx_end_1b, strand=strand_ann, mission6_neg_shift=False).upper()
