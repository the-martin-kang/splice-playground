...
class RefAnnotation:
    ...
    def splice_label_sites_with_kinds_1b(
        self,
        gene: str,
        donor_label_mode: str = "exon_end",
    ) -> Tuple[List[int], List[int], Dict[int, str], Dict[int, str]]:
        """Return donor/acceptor sites AND stable human-readable labels.

        Labels are in transcript exon numbering (5'->3' reading direction):
          - donor for exon i:    exon{i}_donor     (boundary after exon i)
          - acceptor for exon j: exon{j}_acceptor (boundary before exon j)

        This matches the convention used in your preprocessing:
          - acceptor label at exon START (exon j start)
          - donor label at exon END (exon i end)   [or intron_start if selected]

        Note:
          - exon1_acceptor and last_exon_donor are NOT included here (unlabeled during training),
            because we only iterate internal junctions:
              donors: exons 1..(N-1)
              acceptors: exons 2..N
        """
        r = self.get_gene_row(gene)
        ...
        donor_sites: List[int] = []
        acceptor_sites: List[int] = []
        donor_kind_by_1b: Dict[int, str] = {}
        acceptor_kind_by_1b: Dict[int, str] = {}

        n_exons = len(exons)
        for exon_idx, (s, e) in enumerate(exons):
            exon_num = exon_idx + 1

            # acceptor: first base of THIS exon in transcript direction (skip exon1)
            if exon_idx > 0:
                acc = s if strand == "+" else e
                acceptor_sites.append(int(acc))
                acceptor_kind_by_1b[int(acc)] = f"exon{exon_num}_acceptor"

            # donor: boundary after THIS exon (skip last exon)
            if exon_idx < (n_exons - 1):
                if donor_label_mode not in {"intron_start", "exon_end"}:
                    raise ValueError("donor_label_mode must be 'intron_start' or 'exon_end'")

                if donor_label_mode == "intron_start":
                    don = (e + 1) if strand == "+" else (s - 1)
                else:  # exon_end
                    don = e if strand == "+" else s

                donor_sites.append(int(don))
                donor_kind_by_1b[int(don)] = f"exon{exon_num}_donor"

        return donor_sites, acceptor_sites, donor_kind_by_1b, acceptor_kind_by_1b
