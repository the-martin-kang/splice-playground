...
from .constants import IN_LENGTH
...
def apply_alt_at_center(seq_ref: str, alt_base_pos_strand: str, strand: str) -> str:
    ...
    center_idx = len(seq_ref) // 2
    seq_list[center_idx] = alt
...
def build_ref_alt_sequences_from_row(
    ...
    if check_ref and ref_pos and len(ref_pos) == 1 and seq_ref[center_idx] != "N":
        expected = ref_pos
        if strand == "-":
            expected = rc(expected)
        if seq_ref[center_idx] != expected:
            raise ValueError(
                f"Ref base mismatch at center for {chrom}:{pos_1b}({strand}). "
                f"seq_center={seq_ref[center_idx]!r} expected={expected!r} (ref_pos_strand={ref_pos!r})"
            )
